...
bark = sys.argv[14] # barkserver url
...
time_list = [8, 9, 10, 11, 12, 13, 15, 17, 19, 21]
...
set_push = [True, True, True, True, True, True, True, True, True, True]

# 最小步数（如果只需要刷步的次数少于7次就将该次数以后的步数全都改成0，如：time_list[3]: 0，表示第五次开始不运行或者直接云函数触发里面不在该时间调用均可（建议用后者））
min_dict = {time_list[0]: 2000, time_list[1]: 5000, time_list[2]: 8000,time_list[3]: 9000, time_list[4]: 10000, time_list[5]: 20000, time_list[6]: 30000, time_list[7]: 40000, time_list[8]: 50000, time_list[9]: 60000}
# 最大步数（例如现在设置意思是在8点（你设置的第一个时间点默认8）运行会在1500到2999中随机生成一个数提交（开启气候降低步数会乘系数K）10点3000~4999。。。以此类推，步数范围建议看懂了再改，没看懂直接默认就好）
max_dict = {time_list[0]: 4999, time_list[1]: 5999,time_list[2]: 8999,time_list[3]: 9999, time_list[4]: 19999, time_list[5]: 29999, time_list[6]: 39999, time_list[7]: 49999, time_list[8]: 59999, time_list[9]: 69999}
# 设置结束
...
def push_wx(desp=""):
    if sckey == 'NO':
        ...
        server_url = f"{bark}【小米运动步数修改】/{desp}?sound=choo"
